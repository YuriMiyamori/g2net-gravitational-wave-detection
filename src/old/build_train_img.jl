...
function forloop_main(f::String, stype::String, template_signal_fft_arr::Array{ComplexF32,2})
    ...
    for (sig_idx, start_time) in enumerate(0:2:etime-2)
        ...
        isfile(joinpath(save_dir, fname))  && continue 
...
function SNR_main()
    ...
    global save_dir = "/home/yuri/kaggle/g2net-gravitational-wave-detection/dataset/matched_filter_train/"
    ...
    # for stype in ["in_sig", "no_sig"]
        println(stype)
        files = []
        for f in readdir("/home/yuri/kaggle/g2net-gravitational-wave-detection/dataset/train_equalized/", join=true)
            rg ="[0-f]_[0-f]_[0-f]_$(stype)"
            if match(Regex(rg), f) !== nothing # Or if occursin(r"*.\.pdf", f) == true
                push!(files, f)
            end
        end
        # files = sample(files, 1024, replace=false)
        # files = files[1:16]
